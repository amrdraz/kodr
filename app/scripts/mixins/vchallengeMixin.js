...
module.exports = Em.Mixin.create(Em.Evented, {
    ...
    testInServer: function(code, vchallenge, cb) {
        var data = (vchallenge.getProperties(['language','tests', 'exp']));
        data.inputs = vchallenge.get('inputs').mapBy("value");
        Em.$.ajax({
            url: '/api/vchallenges/test',
            type:'POST',
            data: {
                code: code,
                vchallenge: data
